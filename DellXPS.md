...
The root drive node on an XPS 13 with PCI storage is:

    /dev/nvme0n1

    # Make sure to use a zero, not an "O" in nvme0n1
...
Hold F2 (for Dell XPS 13 with touchscreen and F12 for others) during startup to access startup menu. Select the USB drive and boot into Arch.
...
NOTE: The partition nodes will become:
