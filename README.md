...
I typically run Linux on a dual-boot system with Windows. Generally I'll install Windows first, then reduce the size of the partition, freeing up enough unallocated space for Linux. If I'm running a single boot system on either a new drive or a repurposed drive then I will remove all existing partitions after booting from the installation media before partitioning the drive.
